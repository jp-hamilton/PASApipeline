...
my ($opt_c, $opt_C, $opt_r, $opt_R, $opt_A, $opt_g, $opt_t, $opt_f, $opt_T, $opt_u, $opt_d, $opt_h, $opt_x,
	...
	$ANNOTS_FILE, $opt_L, $STRINGENT_ALIGNMENT_OVERLAP, $GENE_OVERLAP, $ac_symlink_prefix,
...
&GetOptions ( 'config|c=s' => \$opt_c,
              ...
              'symlink_prefix=s' => \$ac_symlink_prefix,
...
#  --symlink_prefix <prefix> create symlinks <prefix>.gff3 and <prefix>.bed pointing to the gene structure post PASA update GFF3 and BED files generated by the annotation comparison run
#
...
if ($COMPARE_TO_ANNOT) {
    ...
    if ($ac_symlink_prefix && (-e "${ac_symlink_prefix}.gff3" || -e "${ac_symlink_prefix}.bed" ) ) {
        die "Error: output file ${ac_symlink_prefix}.gff3 or ${ac_symlink_prefix}.bed already exists";
    }
    if ($opt_L) {
    ...
    #create symlinks to the default GFF3 and BED output files if --symlink_prefix is set
    if ($ac_symlink_prefix) {
        $cmd = {
                prog => "ln",
                params => "-s $DBname.gene_structures_post_PASA_updates.$$.gff3 ${ac_symlink_prefix}.gff3",
                input => undef,
                output => undef,
                chkpt => "symlink_annot_compare_gff3_with_prefix.ok" . time(),
               };
        push (@cmds, $cmd);
        $cmd = {
                prog => "ln",
                params => "-s $DBname.gene_structures_post_PASA_updates.$$.bed ${ac_symlink_prefix}.bed",
                input => undef,
                output => undef,
                chkpt => "symlink_annot_compare_bed_with_prefix.ok" . time(),
               };
        push (@cmds, $cmd);
    }
}
